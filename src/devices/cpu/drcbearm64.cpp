...
private:
	...
	struct near_state
	{
		...
		uint8_t vecspill[uml::REG_F_COUNT][16];
	};
	...
	void op_jmpt(a64::Assembler &a, const uml::instruction &inst);
...
void drcbe_arm64::generate(drcuml_block &block, const instruction *instlist, uint32_t numinst)
{
	...
	uint8_t *dst = (uint8_t *)(uint64_t(m_cache.top() + m_linemask) & ~m_linemask);
...
void drcbe_arm64::op_jmpt(a64::Assembler &a, const uml::instruction &inst)
{
	assert(inst.size() == 4);
	assert_no_condition(inst);
	assert_no_flags(inst);
	...
	a.ldrsw(TEMP_REG3, a64::Mem(TEMP_REG1, TEMP_REG2, a64::lsl(2)));
	a.add(TEMP_REG1, TEMP_REG1, TEMP_REG3);
	a.br(TEMP_REG1);
...
void drcbe_arm64::op_vstore(a64::Assembler &a, const uml::instruction &inst)
{
	assert(inst.size() == 4);
	assert_no_condition(inst);
	assert_no_flags(inst);

...
namespace uml {
	...
	enum opcode_t
	{
		...
		OP_JMP,                     // JMP     imm[,c]
		OP_JMPT,                    // JMPT    index,table,count
	...
	public:
		...
		void jmpt(parameter index, const u32 *table, u32 count) { configure(OP_JMPT, 4, index, parameter::make_memory(const_cast<u32 *>(table)), parameter(u64(count))); }

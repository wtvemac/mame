...
inline void solo_asic_video_device::gfxunit_draw_cel(gfx_ymap_t ymap, gfx_cel_t cel, screen_device &screen, bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
	...
	for (int y = y_top; y < y_bottom; y++)
	{
		...
		uint32_t x_start = m_pot_draw_hstart + std::max((uint32_t)0,      ((m_pot_draw_hsize / 2) + ITRUNC(x_left )));
		uint32_t x_end   = m_pot_draw_hstart + std::min(m_pot_draw_hsize, ((m_pot_draw_hsize / 2) + ITRUNC(x_right)));
